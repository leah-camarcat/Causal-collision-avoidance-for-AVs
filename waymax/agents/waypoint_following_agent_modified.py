...
class IDMRoutePolicy(WaypointFollowingPolicy):
    ...
    def update_speed(
        ...
        """Realistic IDM speed update with reaction delay and acceleration limits."""

        log_waypoints = state.log_trajectory
        cur_position = state.current_sim_trajectory.xyz[..., 0, :]
        cur_speed = state.current_sim_trajectory.speed[..., 0]

        # 1. Apply reaction delay to the log trajectory
        delayed_log_waypoints = self._apply_reaction_delay(log_waypoints)

        # 2. Compute ideal IDM acceleration based on delayed perception
        ideal_accel = self._get_accel(
            delayed_log_waypoints, cur_position, cur_speed, state.current_sim_trajectory
        )

        # 3. Clip acceleration to max accel/decel
        accel_delta = jnp.clip(
            ideal_accel, -self.max_decel, self.max_accel
        )

        # 4. Apply first-order lag for realistic acceleration response
        if not hasattr(self, '_prev_accel'):
            self._prev_accel = jnp.zeros_like(accel_delta)
        tau = 0.025  # time constant for lag
        filtered_accel = (dt / tau) * accel_delta + (1 - dt / tau) * self._prev_accel   # first-order low-pass filter
        self._prev_accel = filtered_accel

        # 5. Update speed and ensure non-negative
        speed = cur_speed + dt * filtered_accel
        speed = jnp.maximum(speed, 0.0)

        # 6. Valid mask
        valid = jnp.ones_like(speed, dtype=jnp.bool_)
