...
def get_data_generator(
  ...
  if postprocess_fn is None:
    ...
    for example in dataset.as_numpy_iterator():
      ...
      example_for_jax = example.copy()
      scenario_id = example_for_jax.pop('scenario/id', None)  # 去掉非数组字段
      result = postprocess_fn(example_for_jax)
      # 如果需要，你可以在 result 中再加上 scenario_id
      if hasattr(result, 'object_metadata'):
          result.object_metadata.scenario_id = scenario_id
      yield result
