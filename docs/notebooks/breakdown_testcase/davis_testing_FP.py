...
def detect_intervention(log_vel, sim_vel):
    ...
    if np.sum(sim_vel < 0.1)>=1 and d[np.argmin(sim_vel)] > 1:
        intervened = True
    else: 
        intervened = False
    return intervened
...
filtered_scenarios = [f[:-4] for f in os.listdir("docs/braking_filtered_data") if f.endswith('.pkl')]
decel_values = [-5.884]
...
for shard_idx, shard_file in enumerate(tfrecord_files):
    ...
    for scenario_idx in range(all_counts):
        ...
        for decel in decel_values:
            ...
            # check whether intervention occurred
