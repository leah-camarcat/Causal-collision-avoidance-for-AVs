...
for shard_idx, shard_file in enumerate(tfrecord_files):
    ...
    config = dataclasses.replace(
        ...
        path=shard_file,  # 修改成单个 shard 文件
        max_num_objects=32
    )
    ...
    for scenario_idx in range(all_counts):
        ...
        print(f"Saved video for scenario {scenario_idx}: {scenario_id}, leading vehicles: {leading_vehicles}")

    print(f"All scenarios haven been looped through for the {shard_file.split('/')[-1]}.")