...
def detect_intervention(log_vel, sim_vel):
    #acc_log = jnp.diff(jnp.array(log_vel))/0.1
    #acc_sim = jnp.diff(jnp.array(sim_vel))/0.1
    #min_id = jnp.argmin(acc_sim)
    #d = acc_sim[min_id] - acc_log[min_id]
    sim_vel = jnp.array(sim_vel)
    log_vel = jnp.array(log_vel)
    d = log_vel - sim_vel
    #if acc_sim.any() < -2 and d < 0.5:
    if np.sum(sim_vel < 0.1)>=1 and d[np.argmin(sim_vel)] > 1 and np.sum(sim_vel<1)<5:
        intervened = True
    else: 
        intervened = False
    return intervened
...
for shard_idx, shard_file in enumerate(tfrecord_files):
    ...
    for scenario_idx in range(all_counts):
        ...
        for lcduration in lane_change_values:
            ...
            actor_0 = agents.create_expert_actor(
                dynamics_model=dynamics_model,
            ...
            for i in range(t, T):
                ...
                svx, svy = clean_state.sim_trajectory.vel_x[av_index, i-1], clean_state.sim_trajectory.vel_y[av_index, i-1]
                sv = jnp.linalg.norm(jnp.array([svx, svy]))
                lvx, lvy = clean_state.log_trajectory.vel_x[av_index, i-1], clean_state.log_trajectory.vel_y[av_index, i-1]
                lv = jnp.linalg.norm(jnp.array([lvx, lvy]))
                sim_vel.append(sv)
                log_vel.append(lv)
            ...
            intervened = detect_intervention(log_vel, sim_vel)

            KPIS_dict[lcduration].loc[scenario_id] = pd.Series({
                'false_positive': intervened,
                'log_traj_vel': log_vel,
                'sim_traj_vel': sim_vel,
            })
...
for lcduration in lane_change_values:
    out_csv = f'testing_davis_cutin_FP.csv'
